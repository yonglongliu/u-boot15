...
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0xef);
GC9306_SpiWriteCmd(0x2b);
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0x01);
GC9306_SpiWriteData(0x3f);
GC9306_SpiWriteCmd(0x2c);
GC9306_SpiWriteCmd(0xF0);
GC9306_SpiWriteData(0x02);
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0x00);
GC9306_SpiWriteData(0x02);
GC9306_SpiWriteData(0x06);
GC9306_SpiWriteData(0x00);
...
GC9306_SpiWriteData(0x11);
GC9306_SpiWriteData(0x0A);
GC9306_SpiWriteData(0x3A);//3a
GC9306_SpiWriteData(0x04);
GC9306_SpiWriteData(0x04);
GC9306_SpiWriteData(0x4B);//48

GC9306_SpiWriteCmd(0xF3);
GC9306_SpiWriteData(0x0C);
GC9306_SpiWriteData(0x08);
GC9306_SpiWriteData(0x44);//42
GC9306_SpiWriteData(0x04);
GC9306_SpiWriteData(0x03);
GC9306_SpiWriteData(0x52);//4f

GC9306_SpiWriteCmd(0xF4);
GC9306_SpiWriteData(0x0C);
GC9306_SpiWriteData(0x18);
GC9306_SpiWriteData(0x17);
GC9306_SpiWriteData(0x20);
GC9306_SpiWriteData(0x3B);
GC9306_SpiWriteData(0x0F);

GC9306_SpiWriteCmd(0xF5);
GC9306_SpiWriteData(0x06);
GC9306_SpiWriteData(0x12);
GC9306_SpiWriteData(0x12);
GC9306_SpiWriteData(0x21);
GC9306_SpiWriteData(0x3B);
GC9306_SpiWriteData(0x0F);
...
static unsigned char GC9306_OTP_read_bit(struct panel_spec *self,unsigned char address)
{
	...
	GC9306_SpiWriteCmd(0xe5);
	GC9306_SpiWriteData(0x1c);
	GC9306_SpiWriteCmd(0xe6);
	GC9306_SpiWriteData(address);
	GC9306_SpiWriteCmd(0xe5);
	GC9306_SpiWriteData(0x3c);
	
	GC9306_SpiWriteCmd(0xD5);
        GC9306_SpiRead(lcm_id,1);
        read_value=lcm_id[0];
        LCD_PRINT("GC9306_OTP_read_bit = %d\n",read_value);
        return read_value > 0;
}
...
unsigned char GC9306_OTP_read_byte(struct panel_spec *self,unsigned char address)
{
	unsigned char m=0;
	unsigned char n=0;
	...
	{ 
		n=m<<5;
		n=n|address;
		bit_value = GC9306_OTP_read_bit(self,n) & 0x01;
		otp_data = (bit_value<<m) | otp_data;
 	}
 		
        LCD_PRINT("GC9306_OTP_read_byte = %d\n",otp_data);
 	return otp_data;
}



uint32_t gc9306_manufacture_id(struct panel_spec *self)
{
	...
	GC9306_SpiWriteCmd(0xFE);
	GC9306_SpiWriteCmd(0xEF);
	GC9306_SpiWriteCmd(0xe4);
	GC9306_SpiWriteData(0x05);
	
	ID1 = GC9306_OTP_read_byte(self,0x14);
	ID2 = GC9306_OTP_read_byte(self,0x15);
	ID3 = GC9306_OTP_read_byte(self,0x16);
	
	GC9306_SpiWriteCmd(0xe5);
	GC9306_SpiWriteData(0x00);
	data = ID1<<16|ID2<<8|ID3;
        LCD_PRINT("gc9306_manufacture_id =0x%x \n",data);
...
static int32_t gc9306_read_id(struct panel_spec *self)
{
	...
	gc9306_reset();
	otp_id = gc9306_manufacture_id(self);
	for(i=0;i<4;i++){
		GC9306_SpiWriteCmd(0xFE);
		GC9306_SpiWriteCmd(0xEF);
		...
		if((lcm_id[1] == 0x93 && lcm_id[2] == 0x06) && otp_id != 0x329306)
		{

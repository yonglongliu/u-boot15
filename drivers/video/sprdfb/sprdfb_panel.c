...
extern struct panel_spec lcd_gc9306_sanlong_spi_spec;
extern struct panel_spec lcd_gc9305_holitech_spi_spec;
extern struct panel_spec lcd_st7789_spi_spec;
extern struct panel_spec lcd_gc9306_yihua_spi_spec;
void sprdfb_panel_remove(struct sprdfb_device *dev);

static ushort colormap[256];

static struct panel_cfg panel_cfg[] = {
#ifdef  CONFIG_FB_L278_REL_B191
	{
		.lcd_id = 0x329306,
		.panel = &lcd_gc9306_sanlong_spi_spec,
	},
	{
		.lcd_id = 0x419306,
		.panel = &lcd_gc9306_yihua_spi_spec,
	},
	{
		.lcd_id = 0x119305,
		.panel = &lcd_gc9305_holitech_spi_spec,
	},
	{
		.lcd_id = 0x7789,
		.panel = &lcd_st7789_spi_spec,
	},
	{
		.lcd_id = 0x9305,
...
{
	int id, module_id, i, ret, b_panel_reset=0;
	uint32_t id_adc;
	...
	for(i = 0;i<(sizeof(panel_cfg))/(sizeof(panel_cfg[0]));i++) {
		...
		if(id == panel_cfg[i].lcd_id) {
			...
			if(NULL != dev->panel->ops->panel_readmoduleid){
				module_id = dev->panel->ops->panel_readmoduleid(dev->panel);
				save_lcd_module_id_to_kernel(module_id);
			}

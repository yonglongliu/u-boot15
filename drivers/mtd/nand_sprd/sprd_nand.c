...
void NandSetRandomizer (NAND_CTL * this,uint32_t seedAddr)
{
	...
	__32reg_write(NFC_CFG3_REG,0x00003);//use 8 polynomial and enalbe randomizing
}
...
STATIC_FUNC void nand_set_timing_config(NAND_CTL * this,struct sprd_nand_timing *timing  , uint32_t nfc_clk_MHz) {
	u32 reg_val, temp_val;
		...
		/* get rwh value */
		temp_val = timing->rwh_ns * nfc_clk_MHz / 1000;
		if (((timing->ace_ns * nfc_clk_MHz) % 1000)  != 0) {
			temp_val++;
		}
		//temp_val = 0;
		...
		temp_val = timing->rwl_ns * nfc_clk_MHz / 1000;
		if (((timing->ace_ns * nfc_clk_MHz) % 1000)  != 0) {
			temp_val++;
		}
		//temp_val = 2;
...
int nand_write_spl_page(u8 *buf, struct mtd_info *mtd, u32 page, u32 ecc_pos)
{
	...
	callback_writePageInBlk(nfcBase, page,
					1, NULL,ifHasMBuf, ifHasSBuf,
					MTD_OPS_PLACE_OOB);
	__32reg_write(NFC_CFG3_REG,0x0);//disable random
	return 0;
}
...
#endif
	this->ioaddr = (void __iomem *)SPRD_NANDC_PHYS;
...
STATIC_FUNC int _initGlobeResource(NAND_CTL * this)
{
	...
	//printf("the eb0 before write nandc is %x",sprd_dolphin_reg_read(REG_AON_APB_CLK_EB0));
	//sprd_dolphin_reg_or(REG_AON_APB_CLK_EB0,BIT_AON_APB_NANDC_2X_EB);
	//printf("the eb0 after write nandc is %x",sprd_dolphin_reg_read(REG_AON_APB_CLK_EB0));
	sprd_dolphin_reg_write(REG_AP_CLK_CORE_CGM_NANDC_2X_CFG, BIT(3)); //clk = 390M
	sprd_dolphin_reg_write(REG_AP_CLK_CORE_CGM_NANDC_ECC_CFG, BIT(1));//ecc = 256M

 ...
 *             1 success
 */
OPERATE_STATUS dl_download_start(char *name, uint32_t size, uint32_t nv_checksum, unsigned long checksum_flag)
{
	int ret;
	int index;
...
OPERATE_STATUS dl_read_midst(uint32_t size, uint32_t off, uchar * buf)
{
	...
	static int  read_spl = 0;
	switch (dl_stat.part_type) {
	case PART_TYPE_MTD:
		if(0 == strcmp(dl_stat.mtd.name, "uboot")){
			read_spl ++;
			if(read_spl == 1){

...
#define RD_LENTH 1024*1024
unsigned char rd_eMMCBuf[RD_LENTH];
uint64_t emmc_rd_size = 0;
...
OPERATE_STATUS dl_download_start(uchar * partition_name, uint64_t size, uint32_t nv_checksum, unsigned long checksum_flag)
{
	int i = 0;
	...
	g_status.checksum_flag = checksum_flag;
	if (g_status.checksum_flag) {
		g_status.checksum = nv_checksum;
		g_status.rd_checksum = 0;
	}
...
unsigned long Get_RdEMMC_CheckSum (int len)
{
    unsigned long sum = 0;
	uint64_t off = 0;
	int i = 0;
	...
	emmc_rd_size = g_status.total_recv_size;

	while (emmc_rd_size) {
		if (emmc_rd_size >= RD_LENTH) {
			emmc_rd_size -= RD_LENTH;
		} else {
			len = emmc_rd_size;
			emmc_rd_size = 0;
		}
		src = rd_eMMCBuf;
		if (0 != common_raw_read(g_dl_eMMCStatus.curUserPartitionName, (uint64_t)len, (uint64_t)off, src)) {
			errorf("read error checknum=0x%x!\n",sum);
			return 0;
		}
...
OPERATE_STATUS dl_download_end(void)
{
	...
	if (g_status.checksum_flag) {
		if ((strcmp(g_dl_eMMCStatus.curUserPartitionName,"userdata") == 0)
			||(strcmp(g_dl_eMMCStatus.curUserPartitionName,"l_fixnv1") == 0)
			||(strcmp(g_dl_eMMCStatus.curUserPartitionName,"prodnv") == 0)
                    	||(strcmp(g_dl_eMMCStatus.curUserPartitionName,"usbmsc") == 0)
			||(strcmp(g_dl_eMMCStatus.curUserPartitionName,"cache") == 0)) {
			g_status.total_size = 0;
			return OPERATE_SUCCESS;
		}else {
			g_status.rd_checksum = Get_RdEMMC_CheckSum(RD_LENTH);
			if (g_status.checksum != g_status.rd_checksum) {
				debugf("checksum error\n");
				g_status.total_size = 0;
				return OPERATE_CHECKSUM_DIFF;
			}
		}

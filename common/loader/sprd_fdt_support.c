...
extern uint32_t load_lcd_module_id_to_kernel();
...
int uboot_set_secure_range_paraneters(mem_type type,fdt_addr_t start,fdt_addr_t size){
	...
	smcid  = TEESMC_SIPCALL_WITH_ARG;
	funcid = FUNCTYPE_SET_SECURE_RANGE_PARAM;
...
int fdt_fixup_lcdmoduleid(void *fdt)
{
	char buf[32];

	uint32_t lcd_module_id = 0;
	int str_len;
	int ret;

	lcd_module_id = load_lcd_module_id_to_kernel();
	memset(buf, 0, 32);

	sprintf(buf, "lcd_module_id=ID");
	str_len = strlen(buf);
	sprintf(&buf[str_len], "%x", lcd_module_id);
	str_len = strlen(buf);
	buf[str_len] = '\0';

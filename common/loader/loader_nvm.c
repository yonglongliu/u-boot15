...
int _boot_load_required_image(boot_image_required_t img_info)
{
	...
	if (NULL != img_info.bak_partition) {
		...
		/***secboot 2nd step***/
		secboot_secure_process_flow(img_info.partition,(img_info.size + CERT_SIZE),0,(char *)VERIFY_ADDR);
#if defined (CONFIG_VBOOT_V2)
		memcpy(img_info.mem_addr,VERIFY_ADDR,img_info.size);
#else
...
uint64_t _get_kernel_ramdisk_dt_offset( boot_img_hdr * hdr, uchar *partition)
{
	...
	debug("boot_img_offset.kernel_offset is 0x%llx\n", boot_img_offset.kernel_offset );
...
int load_fixup_dt_img(uchar *partition, uchar **dt_start_addr) {
	...
	if (0 != common_raw_read(partition, (uint64_t)size, (uint64_t)(boot_img_offset.dt_offset + dt_entry_ptr->offset), (char *)fdt_blob)) {
	...
	if (0 != common_raw_read("dtbo", size, 0, &dtbo_header)) {
	...
	if (0 != common_raw_read("dtbo", size, 0, dtbo_addr)) {
		errorf("read dtbo fail\n");
		goto error;
	}
...
/*read dt image*/
	...
	if (0 == load_fixup_dt_img(partition, dt_addr)) {
		if (DT_ADR != *dt_addr) {
...
int secure_get_partition_size(uchar * partition_name, uint64_t * size)
{
	...
	debugf("partition:%s, info.name:%s, info.size:%d, EMMC_SECTOR_SIZE:%d, size:0x%llx\n", partition_name, info.name,  info.size , EMMC_SECTOR_SIZE, *size);
	return ret;
}
#endif
...
void vlx_nand_boot_zebu(char *kernel_pname, int backlight_set, int lcd_enable)
{
    ...
    memcpy((void *)raw_header,(const void *)BOOTIMG_ADR,512*4);
	...
	errorf("A wrong boot.img here,please check!\n");
    }
	free(boot_hdr);
	...
	fdt_fixup_all((uchar *)DT_ADR);
	fdt_initrd_norsvmem((uchar *)DT_ADR,RAMDISK_ADR,RAMDISK_ADR+initrd_size, 1);
...
int sprd_sec_verify_unlockstatus(unsigned char * lockstatus, unsigned int status_len)
{
	...
	ret = uboot_verify_unlockstatus(data_buffer, sizeof(data_buffer));
...
int sprd_sec_verify_password(unsigned char * password, unsigned int pwd_len)
{
#ifndef CONFIG_VBOOT_V2
	return 0; /*default : verify pwd pass.*/
#else
	uint8_t data_buffer[256] __attribute__((aligned(4096)));
	//uint8_t *pwdptr = (uint8_t *)data_buffer + PDT_INFO_UNLOCK_PWD_MAX_SIZE;

	uint32_t ret;
	uint8_t * v_ret = (uint8_t *)data_buffer + PDT_INFO_UNLOCK_PWD_MAX_SIZE * 2;

	if(!password || !pwd_len || (pwd_len > PDT_INFO_UNLOCK_PWD_MAX_SIZE))
	{
		debugf("para error. \n");
		return -1;
	}
	...
	memset(data_buffer, 0, sizeof(data_buffer));
	memcpy(data_buffer, password, pwd_len);

	//get_fbunlockpwd(pwdptr);

#ifdef CONFIG_VBOOT_DUMP
	printf("input pwd:\n");
	do_hex_dump(password, PDT_INFO_UNLOCK_PWD_MAX_SIZE);
#endif

	ret = uboot_verify_pwd(data_buffer, sizeof(data_buffer));

	if(*v_ret == VBOOT_UNLOCK_PWD_CHECK_FAIL)
	{
		debugf("verify password failed. \n");
		ret = -1;
	}
...
void pass_chip_uid_to_tos()
{
    ...
    u32 *blocks = VERIFY_ADDR;

    printf("pass_chip_uid_to_tos()...\n");
    ...
    printf("0x%08x, blocks0: 0x%08x, blk1: 0x%08x\n", blocks, blocks[0], blocks[1]);
    smc_param *param = tee_common_call(FUNCTYPE_SET_CHIP_UID, (uint32_t)blocks, sizeof(u32)*2);
    printf("result: 0x%x \n", param->a0);
#endif
    return;
}
...
void vlx_nand_boot(char *kernel_pname, int backlight_set, int lcd_enable)
{
	...
	/***secboot 2nd step***/
	/*if bootmode is cali mode, do not verify image*/
#if defined	(CONFIG_SOC_IWHALE2)
	char  *boot_mode_type_str;
	unsigned int lcs = 0;
	sprd_get_lcs((unsigned int *)&lcs);
	boot_mode_type_str = getenv("bootmode");
	printf("boot_mode_type=%s\n",boot_mode_type_str);
	if((strncmp(boot_mode_type_str, "cali", 4)) || (SECURE_LCS == lcs))
	{
		printf("partition:%s\n",partition);
		secboot_secure_process_flow(partition,0,0,(char *)VERIFY_ADDR);
	}
#else
...
extern boot_image_required_t *get_cp_load_table(void);
	...
	/***if define CONFIG_KERNEL_BOOT_CP,update swversion is not here***/
#ifndef CONFIG_KERNEL_BOOT_CP
	//secboot_update_swVersion();
#endif
	/***secboot 3rd step***/
	secboot_terminal();
